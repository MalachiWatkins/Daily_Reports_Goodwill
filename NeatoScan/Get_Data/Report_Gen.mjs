...
async function OrderAucion_parse(start, end, type) {
    ...
    parseString(resp_data, async function (err, result) {
        ...
        var delete_doc = await cli_coll.deleteOne(store_del);
        var delete_doc = await cli_coll.deleteOne(delquery);
        console.log("incert");
        var store_ins = await cli_coll.insertOne(store_doc)
...
async function store_PAR(start, end, date_filter) {
    const request_resp = await AuctionRequest(Request_Auction_Inv, 'Inventory', start, end)
    var resp_data = request_resp['data']
    console.log('par');
    parseString(resp_data, async function (err, result) {
...
async function getReport_id(time_filter, start_date, end_date) {
    ...
    const req_xml = await build_request_file(Request_Book_Order_report_XML, 'RequestOrdersReport' , start_date, end_date)
            ...
            async function stat(resp) {
                if (resp['status'] == 'Sent'|| resp['status'] == 'Processing') {
                    ...
                    if (time_filter == 'Daily') {
                        ...
                        const pythonProcess = spawn('python3',["/Users/malachiwatkins/Desktop/work/Daily_Reports_Goodwill/NeatoScan/Get_Data/book_orders_monthly.py"]);
                    }
...
async function auto_run_daily() {
    getReport_id('Daily', daily_start, daily_end)
}
async function auto_run_monthly() {
    getReport_id('Monthly', MTD_Start, MTD_END)
}
...
async function run() {
    ...
    let parc_store = await store_PAR(MTD_Start, daily_end, 'Daily')
    if (daily_start != "01") {
...
async function monthly() {
    ...
    let mtd_parc_store = await store_PAR(MTD_Start, daily_end, 'Monthly')
...
Auction_Activity(MTD_Start, MTD_END, 'Monthly')