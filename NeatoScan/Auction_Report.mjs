...
async function OrderAucion_parse(start, end) {
    ...
    parseString(resp_data, async function (err, result) {
        ...
        //   for (const [key, value] of Object.entries(store_report)) {
        //     let sgw = value['SGW']
        //     let ebay = value['eBay']
        //     console.log(sgw);
        //     console.log(ebay);
        ...
        console.log('SGW Sales: ' + parseFloat(total_subtotal['SGW'][0]));
        console.log('Ebay Sales: ' + parseFloat(total_subtotal['eBay'][0]));
        console.log('PPI: ' + ppi.toFixed(2));
        console.log('Total Sales: ' + total_subtotal_total);
...
async function unshipped() { // All this Does is get Unshipped Orders
    ...
    parseString(resp_data, async function (err, result) {
        ...
        console.log(unshipped_sgw.length);
        console.log(unshipped_ebay.length);
...
async function Auction_Activity(start, end) {
    const request_resp = await AuctionRequest(Request_Auction_Activity, 'Activity', start, end)
    var resp_data = request_resp['data']
    parseString(resp_data, async function (err, result) {
        var json = result;
        ...
        console.log('Postings Per Lister: ' + posting_per_lister);
        console.log('Orders Shipped: ' + order_shipped);
...
//Auction_Refunds(MTD_Start, MTD_END) // Ready to be put in DB
//Auction_Activity(daily_start, daily_end) // Ready for DB //Daily Activity overall and user activity
//auction_activity_log(Neat_start_day,daily_end) // all this does is keep activity log up to date run every 4 hours or so
//store_PAR(MTD_Start, daily_end) // Gets store PARC stats // Ready for db
//unshipped() run this last to get refunds
//Auction_Inventory(MTD_Start,daily_end) // Ready for db 
//OrderAucion_parse(daily_start,daily_end) // This needs to run for daily and monthly